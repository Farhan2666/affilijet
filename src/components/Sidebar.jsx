...
import {
  ...
  Settings, ChevronLeft, ChevronRight, Zap, BarChart3, LogOut, Menu, X
} from 'lucide-react'
...
export default function Sidebar({ activeView, setActiveView }) {
  ...
  return (
    <>
      ...
      <aside className={`fixed left-0 top-0 h-full bg-dark-card border-r border-dark-border z-50 transition-all duration-300 flex flex-col
      ...
      >
        ...
        <div className="p-2 border-t border-dark-border space-y-1">
          <button className="w-full flex items-center gap-3 px-3 py-2.5 rounded-lg text-gray-400 hover:text-red-400 hover:bg-red-500/10 transition-all border border-transparent">
            ...
            {(!collapsed || mobileOpen) && <span className="text-sm font-medium">Disconnect</span>}
          </button>
          <button
            onClick={() => setCollapsed(!collapsed)}
            className="hidden lg:flex w-full items-center justify-center gap-3 px-3 py-2 rounded-lg text-gray-500 hover:text-gray-300 hover:bg-white/5 transition-all"
          >
